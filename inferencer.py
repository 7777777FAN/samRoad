...
def infer_one_img(net, img, config, img_id):
    ...
    for batch_index in range(batch_num):
        ...
        with torch.no_grad():
            ...
            # [B, H, W, 2]
            GTE_scores, patch_img_features = net.infer_masks_and_img_features(batch_img_patches)
    ...
    # decode GTE to gain graph nodes
    GTE_decode_output_dir = os.path.join(output_dir, 'decode_result')
    ...
    graph_adj = DecodeAndVis(fused_GTE_logits_mask, GTE_decode_output_file, thr=0.05, edge_thr=0.05, angledistance_weight=10, snap=True, imagesize=2048)
    ...
    return pred_nodes, pred_edges, fused_keypoint_mask
...
if __name__ == "__main__":
    ...
    for img_id in test_img_indices:
        ...
        # coords in (r, c)
        # try:
            # pred_nodes, pred_edges, keypoint_mask = infer_one_img(net, img, config, img_id)
